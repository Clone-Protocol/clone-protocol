...
describe("depository-token contract tests", async () => {
  ...
  const uniqueSeed = 128;
  const [settingsAccountAddress, _] = PublicKey.findProgramAddressSync(
    [Buffer.from([uniqueSeed]), Buffer.from("settings")],
    programId
  );
  ...
  const depositingTokenAccountAddressSettings = await getAssociatedTokenAddress(
    depositingTokenMintKeypair.publicKey,
  ...
  it(`10 roundtrip mint and redeem tokens`, async () => {
    ...
    for (let i = 0; i < roundtrips; i++) {
      ...
      depositoryTokenBalance = await provider.connection.getTokenAccountBalance(
        depositoryTokenAccountAddress
      );
      depositingTokenBalance = await provider.connection.getTokenAccountBalance(
        depositingTokenAccountAddress
      );
