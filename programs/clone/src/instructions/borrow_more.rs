...
pub struct BorrowMore<'info> {
    ...
    #[account(
        mut,
        has_one = clone,
        constraint = token_data.load()?.pools[borrow_positions.load()?.borrow_positions[borrow_index as usize].pool_index as usize].status == Status::Active as u64 @ CloneError::PoolStatusPreventsAction,
    )]
