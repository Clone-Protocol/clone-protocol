...
pub struct LiquidateCometPosition<'info> {
    ...
    #[account(
        ...
        constraint = user_account.comet.num_collaterals > comet_collateral_index.into() @ CloneError::InvalidInputPositionIndex
    )]
    ...
    #[account(
        has_one = clone,
        constraint = token_data.load()?.pools[user_account.comet.positions[comet_position_index as usize].pool_index as usize].status == Status::Active as u64 ||
        token_data.load()?.pools[user_account.comet.positions[comet_position_index as usize].pool_index as usize].status == Status::Liquidation as u64 @ CloneError::StatusPreventsAction
    )]
    ...
    #[account(
        mut,
        address = token_data.load()?.collaterals[user_account.comet.collaterals[comet_collateral_index as usize].collateral_index as usize].vault,
   )]
...
pub fn execute(
    ...
    let liquidator_fee = Decimal::new(ctx.accounts.clone.liquidator_fee_bps.into(), BPS_SCALE);
