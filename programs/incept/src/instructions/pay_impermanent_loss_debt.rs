...
pub fn execute(
    ...
    let (from_context, mint_context, payment_amount) = if pay_usdi_debt {
        ...
        let payment_amount = rescale_toward_zero(
            (borrowed_usdi - claimable_usdi).min(authorized_amount),
            DEVNET_TOKEN_SCALE,
        );
        ...
        let new_borrowed_amount = rescale_toward_zero(
            comet_position.borrowed_usdi.to_decimal() - payment_amount,
            DEVNET_TOKEN_SCALE,
        );
        ...
        let payment_amount = rescale_toward_zero(
            (borrowed_iasset - claimable_iasset).min(authorized_amount),
            DEVNET_TOKEN_SCALE,
        );
        ...
        let new_borrowed_amount = rescale_toward_zero(
            comet_position.borrowed_iasset.to_decimal() - payment_amount,
            DEVNET_TOKEN_SCALE,
        );
