...
pub fn execute(
    ...
    if comet.is_single_pool == 1 {
        assert!(comet_position_index == comet_collateral_index);
    }
    ...
    if amm_usdi_burn.is_sign_positive() {
        token::burn(
            ...
            amm_usdi_burn.mantissa().try_into().unwrap(),
        )?;
    } else if amm_usdi_burn.is_sign_negative() {
        token::mint_to(
            CpiContext::new_with_signer(
                ctx.accounts.token_program.to_account_info().clone(),
                MintTo {
            ...
            amm_usdi_burn.abs().mantissa().try_into().unwrap(),
        )?;
    }

    if amm_iasset_burn.is_sign_positive() {
        token::burn(
            CpiContext::new_with_signer(
                ctx.accounts.token_program.to_account_info().clone(),
                Burn {
            ...
            amm_iasset_burn.mantissa().try_into().unwrap(),
        )?;
    } else if amm_iasset_burn.is_sign_negative() {
        token::mint_to(
            CpiContext::new_with_signer(
                ctx.accounts.token_program.to_account_info().clone(),
                MintTo {
            ...
            amm_iasset_burn.abs().mantissa().try_into().unwrap(),
        )?;
    }
    ...
    let mut new_collateral_amount =
        comet_collateral.collateral_amount.to_decimal() - collateral_deficit;
